...
void Game::render () {
    ...
    WINDOW* gameWindow = newwin(b_h+2, b_w*2+2, 1, 2);
    start_color();
    ...
    WINDOW* statsWindow = newwin(4, 19, 1, b_w*2 +10);
    box(statsWindow, 0, 0);
    ...
    WINDOW* nextWindow = newwin(7, 15, 7, b_w*2+12);
    box(nextWindow, 0, 0);
    for (int i = 1; i < 5; ++i) {
        for (int j = 3; j < 7; ++j) {
...
void Game::trasformTetromino (int key) {
    switch (key) {
        ...
        case KEY_DOWN:
            ...
            break;
	case ' ':
	    while(tetromino.moveDown() && !collideWithTetrominoes());
            if (collideWithTetrominoes()) tetromino.moveUp();
	    break;
    }
}
